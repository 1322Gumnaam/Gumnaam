...
def get_user_info(message):
    ...
    if user_id in subscriptions:
        ...
        if datetime.datetime.now() < expiry:
            subscription_status = f"Active until {expiry.strftime('%Y-%m-%d %H:%M:%S')}"
        else:
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 5000:
                ...
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Time: {time}" # Notify the user that the attack is finished
        else:
...
def handle_start(message):
    ...
    if user_id in allowed_user_ids:
        if is_subscribed(user_id):
            response = "Welcome back! You are already subscribed."
        else:
            response = "Your subscription has expired. Please renew your subscription."
    else:
        response = "You are not authorized to use this bot. Please contact the admin to get access."
...
def handle_stop(message):
    ...
    if user_id in allowed_user_ids:
        if is_subscribed(user_id):
            response = "Your subscription is still active. To stop using the service, contact the admin."
        else:
            response = "Your subscription has expired. You can no longer use this service."
    else:
    ...
    if user_id in allowed_user_ids and is_subscribed(user_id):
        ...
        if len(command) > 1:
            ...
            try:
                ...
                record_command_logs(user_id, shell_command, time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                response = f"Command executed:\n\n<code>{output}</code>"
            except Exception as e:
                response = f"Error executing command: {str(e)}"
        else:
            response = "Please provide a shell command to execute."
    else:
